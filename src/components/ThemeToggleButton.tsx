...
export default function ThemeToggleButton({ compact = false }: ThemeToggleButtonProps) {
  ...
  const containerClass = compact
    ? 'relative flex h-8 w-[58px] items-center rounded-[0.9rem] border border-white/10 bg-white/[0.06] p-1 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] backdrop-blur-xl shadow-[0_10px_26px_rgba(0,0,0,0.16)] hover:border-white/20 sm:h-10 sm:w-[70px] sm:rounded-[1rem] sm:p-1.5'
  ...
  const innerTrackClass = compact ? 'absolute inset-[3px] rounded-[0.72rem] sm:rounded-[0.82rem]' : 'absolute inset-[3px] rounded-[0.95rem]';
  const iconClass = compact ? 'h-3 w-3 sm:h-3.5 sm:w-3.5' : 'h-4 w-4';
  const thumbBaseClass = compact
    ? 'absolute left-1 top-1 flex h-6 w-6 items-center justify-center rounded-[0.68rem] border shadow-[0_8px_18px_rgba(15,23,42,0.2)] transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] sm:left-1.5 sm:top-1.5 sm:h-7 sm:w-7 sm:rounded-[0.75rem]'
  ...
  const thumbTranslateClass = compact
    ? (isLight ? 'translate-x-[26px] sm:translate-x-[28px]' : 'translate-x-0')
  ...
  const thumbIconClass = compact ? 'h-3.5 w-3.5 sm:h-4 sm:w-4' : 'h-[18px] w-[18px]';
